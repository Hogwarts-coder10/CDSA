...
int main() {
  ...
  cdsa_ringbuffer *rb = cdsa_create_ringbuffer(3, sizeof(int));

  // 1. Fill the buffer
  printf("[*] Testing Push (Filling Buffer)...\n");
  int a = 10, b = 20, c = 30;

  printf("Push 10: %s\n", cdsa_push_back_ringbuffer(rb, &a) ? "Success" : "Failed");
  printf("Push 20: %s\n", cdsa_push_back_ringbuffer(rb, &b) ? "Success" : "Failed");
  printf("Push 30: %s\n", cdsa_push_back_ringbuffer(rb, &c) ? "Success" : "Failed");

  printf("Is Full? %s (Expected: Yes)\n\n",
         cdsa_is_full_ringbuffer(rb) ? "Yes" : "No");

  // 2. Overfill guard
  printf("[*] Testing Overfill Guard...\n");
  int d = 40;
  printf("Push 40: %s (Expected: Failed)\n\n",
         cdsa_push_back_ringbuffer(rb, &d) ? "Success" : "Failed");

  // 3. The Wrap-Around Magic
  printf("[*] Testing Wrap-Around (Pop 1, Push 1)...\n");
  printf("Front item: %d (Expected: 10)\n", *(int *)cdsa_front_ringbuffer(rb));

  cdsa_pop_front_ringbuffer(rb); // Removes 10, frees up a slot
  printf("Popped front. Size is now: %zu\n", cdsa_size_ringbuffer(rb));
  ...
  while (!cdsa_is_empty_ringbuffer(rb)) {
    printf("Popping: %d\n", *(int *)cdsa_front_ringbuffer(rb));
    cdsa_pop_front_ringbuffer(rb);
  }

  printf("Is Empty? %s (Expected: Yes)\n",
         cdsa_is_empty_ringbuffer(rb) ? "Yes" : "No");
